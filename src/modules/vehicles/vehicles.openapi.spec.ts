...
describe('Vehicles OpenAPI contract', () => {
  ...
  it('publishes vehicle fields and pagination metadata', () => {
    const vehicle = document.components?.schemas
      ?.VehicleResponseDto as OpenApiSchema | undefined;
    const page = document.components?.schemas
      ?.VehiclePageResponseDto as OpenApiSchema | undefined;
  ...
  it('documents search, page, and bounded limit query parameters on list', () => {
    ...
    expect(
      parameters.map((p) => 'name' in p && p.name),
    ).toEqual(expect.arrayContaining(['customerId', 'search', 'page', 'limit']));
  });
});
